...
- 只有 MCP 不可用时，`inspect_metadata.py`、`resolve_datasource.py`、`get_table_ddl.py`、`query_opendataworks_metadata.py`、`run_sql.py` 才作为兼容 fallback 通过 skill 自带 `${DATAAGENT_SKILL_ROOT}/bin/odw-cli` 调 backend agent API。
...
- 用户已经给出明确表名时，直接进入这条 SQL 模板对应的只读查询步骤，不要搜索仓库里的 lineage/血缘代码实现。
