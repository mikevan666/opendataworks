...
- 只有 MCP 不可用时，`inspect_metadata.py`、`resolve_datasource.py`、`query_opendataworks_metadata.py`、`run_sql.py` 才作为兼容 fallback 通过 skill 自带 `${DATAAGENT_SKILL_ROOT}/bin/odw-cli` 调 backend agent API。
