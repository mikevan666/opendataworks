...
const REASONING_START = '[[ODA_REASONING_START]]'
const REASONING_END = '[[ODA_REASONING_END]]'
...
const toolStatusFromEvent = (eventType) => {
  ...
  if (eventType === 'tool.output') return 'streaming'
...
export const createAssistantMessageState = (seed = {}) => ({
  ...
  _reasoningParsers: Object.create(null)
})

const markerPrefixCarry = (text, marker) => {
  const source = textOrEmpty(text)
  const target = textOrEmpty(marker)
  const max = Math.min(source.length, target.length - 1)
  for (let size = max; size > 0; size -= 1) {
    if (source.endsWith(target.slice(0, size))) return size
  }
  return 0
}

const ensureReasoningParser = (msg, blockId) => {
  const key = textOrEmpty(blockId).trim()
  if (!key) return null
  if (!msg._reasoningParsers[key]) {
    msg._reasoningParsers[key] = {
      mode: '',
      buffer: '',
      justClosedReasoning: false
    }
  }
  return msg._reasoningParsers[key]
}

const appendReasoningMainText = (msg, blockId, text) => {
  const chunk = textOrEmpty(text)
  if (!chunk) return
  const block = ensureTextBlock(msg, blockId, 'main_text')
  block.status = 'streaming'
  block.text = appendStr(block.text, chunk)
  syncMainText(msg)
}

const appendReasoningThinkingText = (msg, blockId, text) => {
  const chunk = textOrEmpty(text)
  if (!chunk) return
  const block = ensureTextBlock(msg, `${blockId}:thinking`, 'thinking')
  block.status = 'streaming'
  block.text = appendStr(block.text, chunk)
  syncThinkingText(msg)
}

const emitReasoningPartial = (msg, blockId, parser, marker, flush, thinking) => {
  const keep = flush ? 0 : markerPrefixCarry(parser.buffer, marker)
  if (!flush && parser.buffer.length <= keep) return false
  const emit = flush ? parser.buffer : parser.buffer.slice(0, parser.buffer.length - keep)
  parser.buffer = flush ? '' : parser.buffer.slice(parser.buffer.length - keep)
  if (!emit) return false
  if (thinking) appendReasoningThinkingText(msg, blockId, emit)
  else appendReasoningMainText(msg, blockId, emit)
  return true
}

const drainReasoningParser = (msg, blockId, flush = false) => {
  const parser = ensureReasoningParser(msg, blockId)
  if (!parser) return
  while (true) {
    if (parser.mode === 'thinking') {
      const endIndex = parser.buffer.indexOf(REASONING_END)
      if (endIndex >= 0) {
        appendReasoningThinkingText(msg, blockId, parser.buffer.slice(0, endIndex))
        parser.buffer = parser.buffer.slice(endIndex + REASONING_END.length)
        parser.mode = ''
        parser.justClosedReasoning = true
        continue
      }
      if (!emitReasoningPartial(msg, blockId, parser, REASONING_END, flush, true)) return
      continue
    }
    ...
    if (parser.justClosedReasoning) {
      ...
      parser.buffer = parser.buffer.replace(/^\n/, '')
      if (!parser.buffer && !flush) return
      parser.justClosedReasoning = false
    }

    const startIndex = parser.buffer.indexOf(REASONING_START)
    if (startIndex >= 0) {
      appendReasoningMainText(msg, blockId, parser.buffer.slice(0, startIndex))
      parser.buffer = parser.buffer.slice(startIndex + REASONING_START.length)
      parser.mode = 'thinking'
      continue
    }

    if (!emitReasoningPartial(msg, blockId, parser, REASONING_START, flush, false)) return
  }
}

const appendReasoningTextChunk = (msg, blockId, text) => {
  const parser = ensureReasoningParser(msg, blockId)
  if (!parser) return
  parser.buffer += textOrEmpty(text)
  drainReasoningParser(msg, blockId, false)
}

const flushReasoningTextBlock = (msg, blockId) => {
  const parser = msg._reasoningParsers?.[blockId]
  if (!parser) return
  drainReasoningParser(msg, blockId, true)
  delete msg._reasoningParsers[blockId]

  const mainBlock = msg._renderBlockMap[blockId]
  if (mainBlock && mainBlock.status !== 'failed') {
    mainBlock.status = 'success'
  }
  const thinkingBlock = msg._renderBlockMap[`${blockId}:thinking`]
  if (thinkingBlock && thinkingBlock.status !== 'failed') {
...
export const processAssistantStreamEvent = (msg, event) => {
  ...
  if (type === 'content_block_start') {
    ...
    if (contentType === 'text') {
      if (contentBlock.text) {
        appendReasoningTextChunk(msg, blockId, contentBlock.text)
      } else {
        ensureTextBlock(msg, blockId, 'main_text').status = 'streaming'
      }
      return
    }
  ...
  if (type === 'content_block_delta') {
    ...
    if (deltaType === 'text_delta') {
      appendReasoningTextChunk(msg, blockId, delta.text)
      return
    }
  ...
  if (type === 'content_block_stop') {
    const index = event.index ?? payload.index
    const blockId = resolveRenderBlockId(`cb-${index}`)
    flushReasoningTextBlock(msg, blockId)
    const block = msg._renderBlockMap[blockId]
    if (!block) return
  ...
  if (type === 'message_delta') {
    ...
    const usage = isPlainObject(event.usage)
      ? event.usage
      : (isPlainObject(delta.usage) ? delta.usage : null)
    ensureMessageMeta(msg, ensureActiveMessageKey(), {
  ...
  if (type === 'message_stop') {
    ...
    if (!['tool_use', 'tool_calls'].includes(textOrEmpty(msg.stop_reason).trim())) {
      msg.status = 'success'
    }
    return
  }
  ...
  if (type === 'agent_stop') {
    if (!['failed', 'cancelled'].includes(textOrEmpty(msg.status).trim())) {
      msg.status = 'success'
    }
    markAllStreamingBlocksComplete(msg)
    return
  ...
  if (type === 'text.delta') {
    appendReasoningTextChunk(msg, 'main-text', payload.text)
    return
  }
  ...
  if (type === 'text.complete') {
    ...
    flushReasoningTextBlock(msg, 'main-text')
    return
  }
