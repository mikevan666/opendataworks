...
仓库内置了基于样例数据的前端演示模式，不依赖本地 Spring Boot 和 MySQL。
...
- 仓库根目录已提供 [vercel.json](vercel.json)，静态输出目录为 `frontend/dist`。
- GitHub Actions 工作流位于 [.github/workflows/vercel-demo-deploy.yml](.github/workflows/vercel-demo-deploy.yml)，仅在 `push main` 和手动触发时部署。
- 需要在 GitHub 仓库 Secrets 中配置 `VERCEL_TOKEN`、`VERCEL_ORG_ID`、`VERCEL_PROJECT_ID`。
- 推荐将该 Vercel 项目作为独立项目由 CLI / API 部署，不启用 Git Integration，这样不会产生其他分支的预览部署。
